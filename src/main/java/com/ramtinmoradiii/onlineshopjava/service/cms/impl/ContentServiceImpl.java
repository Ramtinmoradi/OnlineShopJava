...
public class ContentServiceImpl implements ContentService {
    ...
    public ContentResponse readByName(String name) {
        return mapper.map(
                repository.findByName(name).orElseThrow(() -> new NotFoundException("ایتم مورد نظر یافت نشد.")),
    ...
    public ContentResponse readById(Long id) {
        return mapper.map(
                repository.findById(id).orElseThrow(() -> new NotFoundException("ایتم مورد نظر یافت نشد.")),
                ContentResponse.class
        );
    }
    ...
    public ContentResponse update(Long id, ContentRequest request) {
        return mapper.map(
                repository.save(repository.findById(id).orElseThrow(() -> new NotFoundException("ایتم مورد نظر یافت نشد."))),
                ContentResponse.class
        );
    }
    ...
    public void deleteById(Long id) {
        if (!repository.existsById(id)) throw new NotFoundException("ایتم مورد نظر یافت نشد.");
